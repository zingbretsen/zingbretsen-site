...
const TodoPage = () => {
  ...
  function addTodo() {
    ...
    const newTodo = API.graphql(
      ...
      .then((d) => {
        console.log(d.data.createTodo);
        changeState([d.data.createTodo, ...state]);
      })
      .catch((e) => console.log(e));
  }

  useEffect(() => {
    if (count > 0) document.title = count;
  });
  ...
  return (
    <Layout title="Todos">
      ...
      <input
        type="text"
        onChange={(d) => changeName(d.target.value)}
        name="name"
        placeholder="name"
      />
      <input
        type="text"
        onChange={(d) => changeDescription(d.target.value)}
        name="description"
        placeholder="description"
      />
      {state.map((todo) => (
        <Todo key={todo.id} title={todo.name} description={todo.description}>
          <p>{todo.name}</p>
        </Todo>
      ))}
    </Layout>
